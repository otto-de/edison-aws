...
import de.otto.edison.aws.dynamodb.configuration.DynamoProperties;
...
public abstract class AbstractDynamoRepository<V> {
    ...
    public Stream<V> findAllAsStream(final String lastKey) {
        return toStream(table()
                .scan(new ScanSpec().withExclusiveStartKey(getKeyFieldName(), lastKey)))
                .map(this::decode);
    }
    ...
    public void deleteAll() {
        table().scan(new ScanSpec().withAttributesToGet(getKeyFieldName())).forEach(item -> {
            final String key = item.getString(getKeyFieldName());
            table().deleteItem(getKeyFieldName(), key);
        });
    }
